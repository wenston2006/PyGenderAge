...
model_weights = './squeeze_gender_age_dp_0124__iter_50000.caffemodel'
...
for itt in range(len(indexlist)):
    ...
    ests_gender = ests[0, 0:2]
    ests_gf = np.array([0] * 2)
    # gender_id = ests_gender.argmax()
    # ests_gf[gender_id] = 1
    ests_gf[ests_gender.argmax()] = 1

    pred_equal = hamming_distance(gts_gender, ests_gf)

    ests_gender_sig = sigmoid(ests_gender)
    if ests_gender_sig[ests_gender.argmax()] < 0.6 and pred_equal == 1:
        pred_equal = 0
